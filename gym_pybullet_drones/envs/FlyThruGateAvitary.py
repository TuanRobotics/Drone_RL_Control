...
class FlyThruGateAvitary(BaseRLAviary):
    def __init__(self,
                ...
                use_physic_randomization: bool = False,
        ...
        # Setup for randomization physic paramters
        self.use_physic_randomization = use_physic_randomization
        ...
        self._cache_nominal_params()
        
    # Set up for randomization parameters of Drone 
    def _cache_nominal_params(self):
        #  BaseAviary after super().__init__()
        self._M0 = float(self.M)
        self._J0 = self.J.copy()          # (3,3)
        self._KF0 = float(self.KF)
        self._KM0 = float(self.KM)
        self._DRAG0 = self.DRAG_COEFF.copy() if hasattr(self, "DRAG_COEFF") else None
        self._DW10 = float(self.DW_COEFF_1) if hasattr(self, "DW_COEFF_1") else None
        self._DW20 = float(self.DW_COEFF_2) if hasattr(self, "DW_COEFF_2") else None

    def _update_derived_dynamics(self):
        """Recompute derived constants after changing physical parameters."""
    ...
    def _addObstacles(self): 
        ...
        # size = np.array(aabb_max) - np.array(aabb_min)  # [x, y, z] in the world frame
    ...
    # Function apply randomization for physic parameters of Drone 
    def _apply_domain_randomization(self):
        rng = getattr(self, "np_random", np.random.default_rng())

        # 1) Sample scales
        m_scale  = rng.uniform(0.90, 1.10)     # mass +-10%
        j_scale  = rng.uniform(0.85, 1.10)     # inertia +-15%
        kf_scale = rng.uniform(0.90, 1.10)     # thrust coeff +-10%
        km_scale = rng.uniform(0.90, 1.10)     # torque coeff +-10%
        drag_scale = rng.uniform(0.8, 1.0)     # drag scale

        # 2) Keep thrust-to-weight roughly stable by scaling KF with mass
        kf_scale *= m_scale

        # 3) Update model parameters used by the environment
        self.M  = self._M0 * m_scale
        self.J  = self._J0 * j_scale
        self.J_INV = np.linalg.inv(self.J)

        self.KF = self._KF0 * kf_scale
        self.KM = self._KM0 * km_scale

        if self._DRAG0 is not None:
            self.DRAG_COEFF = self._DRAG0 * drag_scale
        if self._DW10 is not None:
            self.DW_COEFF_1 = self._DW10 * rng.uniform(0.9, 1.1)
        if self._DW20 is not None:
            self.DW_COEFF_2 = self._DW20 * rng.uniform(0.9, 1.1)
        self._update_derived_dynamics()

        # 4) Push updated parameters into PyBullet dynamics
        for drone_id in self.DRONE_IDS:
            # base link index = -1
            p.changeDynamics(
                drone_id, -1,
                mass=self.M,
                linearDamping=rng.uniform(0.0, 0.04),
                angularDamping=rng.uniform(0.0, 0.04),
                lateralFriction=rng.uniform(0.6, 1.2),
                spinningFriction=rng.uniform(0.0, 0.02),
                rollingFriction=rng.uniform(0.0, 0.02),
                physicsClientId=self.CLIENT
            )

            # Try setting inertia diagonal if the PyBullet build supports it
            try:
                diag = np.array([self.J[0,0], self.J[1,1], self.J[2,2]], dtype=float)
                p.changeDynamics(
                    drone_id, -1,
                    localInertiaDiagonal=diag.tolist(),
                    physicsClientId=self.CLIENT
                )
            except Exception:
                pass
    ...
    def reset(self, seed=None, options=None):
        ...
        self.INIT_RPYS[0, 2] = 0.2 * (rng.random() - 0.5)
        ...
        if self.use_curriculum:
            ...
            if lvl == 0:
                ...
                forward_offset = 0.2
                lateral_noise = 0.01 * (rng.random() - 0.5)
                vertical_noise = 0.01 * (rng.random() - 0.5)
                self.INIT_RPYS[0, 2] = 0.0
            elif lvl == 1:
                # Level 1: Near the gate, small noise
                forward_offset = 0.3
                lateral_noise = 0.05 * (rng.random() - 0.5)
                vertical_noise = 0.05 * (rng.random() - 0.5)
                self.INIT_RPYS[0, 2] = 0.0
                ...
                forward_offset = 0.8
                lateral_noise = 0.1 * (rng.random() - 0.5)
                vertical_noise = 0.1 * (rng.random() - 0.5)
                self.INIT_RPYS[0, 2] = 0.0
                ...
                forward_offset = 1.2
                lateral_noise = 0.1 # 0.5 * (rng.random() - 0.5)
                vertical_noise = 0.1 #0.5 * (rng.random() - 0.5)
                self.threshold_success = 0.08
                # Can add random yaw orientation
                self.INIT_RPYS[0, 2] = 0.2 # 0.2 * (rng.random() - 0.5)
        ...
        if self.use_physic_randomization:
    ...
    def _computeInfo(self):
        ...
        info = {
            ...
            "quaternion": state[3:7].tolist(),
            "rpy": state[7:10].tolist(),
            "lin_vel": state[10:13].tolist(),
            "ang_vel": state[13:16].tolist(),
            "time_passed_gate": float(self.time_passed_gate),
            "rand_params": {
                "M": float(self.M),
                "KF": float(self.KF),
                "KM": float(self.KM),
