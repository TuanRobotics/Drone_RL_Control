...
class SACAgent:
    ...
    def save(self, filepath):
        # Also persist optimizers and entropy params so we can resume training smoothly
        log_alpha_state = None
        alpha_optimizer_state = None
        alpha_value = None
        if hasattr(self, "log_alpha"):
            log_alpha_state = self.log_alpha.detach().cpu()
            alpha_value = self.alpha.detach().cpu() if torch.is_tensor(self.alpha) else torch.tensor(self.alpha)
    ...
    def load(self, filepath, load_optimizer=True):
        ...
        # Restore entropy parameters if available
        if self.automatic_entropy_tuning and hasattr(self, "log_alpha"):
            log_alpha_state = checkpoint.get('log_alpha')
            if log_alpha_state is not None:
                self.log_alpha.data.copy_(torch.as_tensor(log_alpha_state, device=device))
                self.alpha = self.log_alpha.exp()
            else:
                alpha_value = checkpoint.get('alpha')
                if alpha_value is not None:
                    alpha_tensor = torch.as_tensor(alpha_value, device=device)
                    self.alpha = alpha_tensor

        # Optionally restore optimizer states to truly resume training
        if load_optimizer:
            ...
            if self.automatic_entropy_tuning and hasattr(self, "alpha_optimizer"):
