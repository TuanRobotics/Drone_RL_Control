...
def train_ddpg(num_episodes=10000,
               learn_every=1,
               warmup_steps=10000,
               eval_every=50,
               num_eval_episodes=5,
               gui=False):
    """Train DDPG agent with TD3/SAC-style logging and plots."""
    ...
    save_dir = Path(base_dir) / "ddpg_training_thrugate" / f"ddpg_{timestamp}"
    save_dir.mkdir(parents=True, exist_ok=True)
    csv_prefix = Path(base_dir) / f"ddpg_metrics_{timestamp}"

    print("\n" + "=" * 60)
    print("Starting DDPG Training for Drone Gate Navigation")
    print("=" * 60 + "\n")
    ...
    for episode in range(num_episodes):
        ...
        if episode % 1000 == 0 and episode > 0:
            agent.save(os.path.join(save_dir, f"ddpg_model_ep{episode}.pt"))
    ...
    print(f"{'='*60}\n")
    agent.save(os.path.join(save_dir, "ddpg_model_final.pt"))
    plot_training_curves(episode_rewards, eval_rewards, critic_losses,
...
def evaluate_policy(env, agent, num_episodes=5):
    """Evaluate DDPG policy without exploration noise."""
    eval_rewards = []
...
def plot_training_curves(episode_rewards, eval_rewards, q_losses,
    ...
    if q_losses:
        axes[1, 0].plot(q_losses, alpha=0.3)
        if len(q_losses) >= 100:
            smoothed = np.convolve(q_losses, np.ones(100)/100, mode='valid')
            axes[1, 0].plot(range(99, len(q_losses)), smoothed, linewidth=2)
    ...
    if policy_losses:
        ...
        if len(policy_losses) >= 100:
            smoothed = np.convolve(policy_losses, np.ones(100)/100,
                                   mode='valid')
            axes[1, 1].plot(range(99, len(policy_losses)), smoothed,
                            linewidth=2)
