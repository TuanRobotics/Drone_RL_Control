#!/usr/bin/env python3
"""Train TD3 on the curriculum-based FlyThruGate environment."""
...
from agents.td3_agent import Actor, Critic, TD3Agent
...
class TrainConfig:
    ...
    run_name: str = "td3_thrugate_curriculum"
    log_root: str = "/home/tuan/Desktop/drone_rl_control/log_dir/td3_training_thrugate_curriculum"


class SuccessCurriculum:
    """Tracks success rate and increments level when threshold met."""
...
def evaluate_policy(env: FlyThruGateCurriculumAvitary, agent: TD3Agent, episodes: int) -> float:
    rewards = []
...
def train(cfg: TrainConfig):
    ...
    for episode in range(cfg.num_episodes):
        ...
        if (episode + 1) % (cfg.eval_every * 4) == 0:
            ckpt_path = ckpt_dir / f"step_{total_steps}_td3.pth"
            agent.save(str(ckpt_path))

    agent.save(str(ckpt_dir / "final_td3.pth"))
